...
function createWeatherIcon(loc: RouteLocation): L.DivIcon {
...
function MapBounds({ locations }: { locations: RouteLocation[] }) {
 const map = useMap();
 useEffect(() => {
...
export default function RouteMap({ locations, source, destination }: RouteMapProps) {
 ...
 return (
  ...
  <h2 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2 group-hover:text-blue-500 transition-colors duration-300">
 ...
 <span className="flex items-center gap-1">
 <span className="inline-block w-3 h-3 rounded-full bg-[#3b82f6]" /> Waypoint
 </span>
 <span className="flex items-center gap-1">
