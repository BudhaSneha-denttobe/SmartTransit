import type { RouteInfo, HourlyForecast, DailyForecast } from '../types';

export const mockRouteData: RouteInfo = {
  source: 'Vijayawada',
  destination: 'Guntur',
  distance: '35 km',
...
function generateMockHourly(): HourlyForecast[] {
  const now = new Date();
...
export const mockHourlyForecast: HourlyForecast[] = generateMockHourly();

export function generateMockDailyForecast(): DailyForecast[] {
  const now = new Date();
  const data: { day: string; high: number; low: number; condition: string; rain: number; icon: string }[] = [
