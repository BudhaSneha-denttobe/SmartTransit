...
async function geocodeOsm(query) {
  ...
  try {
    const res = await fetch(url, {
      headers: { 'User-Agent': 'SmartTransitBackend/1.0' },
    });
...
function interpolatePoints(lat1, lng1, lat2, lng2, count) {
  const points = [];
...
function buildHourlyFromNow(raw, hours) {
...
async function fetchForecast(lat, lng, currentWeather) {
  const key = getWeatherKey();
  if (!key) return null;
  ...
  const raw = data.list
    .filter((item) => item.dt >= now && item.dt <= twelveHoursLater)
    .slice(0, 5)
    .map((item) => ({
      dt: item.dt,
  ...
  if (currentWeather) {
    raw.unshift({
      ...
      rainChance: currentWeather.rainProbability,
...
exports.getRouteWeather = async (req, res) => {
  try {
    ...
    const key = getWeatherKey();
    if (!key) {
      return res.status(500).json({ message: 'OPENWEATHER_API_KEY not configured on server' });
    }
    ...
    if (!srcCoords && !dstCoords) {
      return res.status(404).json({ message: `Could not find "${source}" or "${destination}". Try different city names.` });
    }
    if (!srcCoords) {
      return res.status(404).json({ message: `Could not find location: "${source}". Try a different name.` });
    }
    if (!dstCoords) {
      return res.status(404).json({ message: `Could not find location: "${destination}". Try a different name.` });
    }
